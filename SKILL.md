...
description: Redacta dictámenes periciales de patologías constructivas en .docx, con el estilo configurable del perito (definido en `boilerplate/`). Úsalo cuando el usuario pida crear un encargo nuevo, redactar, presupuestar o revisar un informe pericial. Trigger: "informe pericial", "dictamen pericial", "informe de patologías", "encargo nuevo", "redacta el informe del [caso]", "/informe ...".
---

# Skill: informe-pericial

Genera dictámenes periciales en .docx siguiendo una estructura y estilo configurables.

**Ubicación de los encargos:** cada encargo se crea como subcarpeta del **directorio de trabajo actual** (cwd) cuando se invoca `/informe nuevo`. el usuario hace `cd` a donde quiere que viva el encargo (ej. `Z:\03-INFORME\AÑO 2026\`) y allí se crea `<YYYY_NNN_apellido_localidad>/`. NO hay raíz fija predeterminada.

## Flujo de trabajo

```
1. /informe nuevo               → conversación guiada → carpeta + caso.yaml + objeto_pericial.md
2. (el usuario vuelca documentación, fotos, notas Notability)
...
6. (el usuario mide en Presto, exporta los 2 docx a 99_salida/presupuesto/)
7. /informe presupuestar        → inyecta capítulos en cada deficiencia → v2
8. (el usuario revisa en Word)
...
- **Cada deficiencia se redacta sin coste primero (v1).** Los costes entran en v2 desde los docx que el usuario exporta.
...
1. **3 preguntas mínimas, una por turno**, en lenguaje natural:
   ...
   - **"¿Código del encargo? (formato YYYY_NNN, ej. 2026_007)"** — Si el usuario dice "el siguiente" o "no sé", escanea las carpetas hermanas en cwd con patrón `YYYY_NNN_*` del año actual y propón el siguiente número libre.
...
Procesa todo lo que el usuario ha volcado tras `/informe nuevo`. Se puede ejecutar varias veces conforme se añade material; es idempotente.
...
1. **"Cuéntame en 2-3 frases: ¿qué te ha pedido el cliente y qué problema hay con la obra?"**
   → Esto pasa a `00_encargo/objeto_pericial.md` (el usuario puede afinar luego).
...
3. **Extracción autónoma de datos al `caso.yaml`** — leer los `.txt` relevantes (hoja de encargo, demanda, contrato, proyecto, escritura, informe contrario) y rellenar TODO lo que se pueda:
   ...
   **Regla de oro**: si un dato aparece en un PDF, NO se le vuelve a preguntar al usuario. Sólo se confirma al final si hay ambigüedad (ej. dos direcciones distintas en dos documentos).
...
3 bis. **MEMORIA EXTERNA: notas por documento + cronología** *(crítico para encargos con mucha documentación)*
   ...
   **b) `_skill_workspace/cronologia.md`** — línea de tiempo del caso construida cruzando fechas de todos los documentos. Formato exacto:
   ...
   - Hechos relevantes: contratos, actas de obra, certificaciones, comunicaciones (burofax, requerimientos), recepciones, daños reportados, peritajes previos, demandas, visitas del usuario, encargo. NO incluir trivialidades.
...
4. **Procesar fotos** (origen: carpeta de Drive del encargo, vía Google Drive for Desktop). Flujo nuevo basado en Excel:
   ...
   **Paso 4.1 — Copiar y renombrar:**
   - Preguntar al usuario la ruta de la carpeta de Drive del encargo (típicamente bajo `G:\Mi unidad\...`).
   ...
   - **Pre-requisito clave**: para que el dropdown de Deficiencia del Excel salga útil, conviene que `deficiencias.md` esté ya creado (modo borrador es suficiente — el usuario puede ajustarlo en `/informe deficiencias`). Si no existe aún, el Excel se genera sin dropdown y el usuario teclea los slugs a mano.

   **Paso 4.2 — el usuario rellena el Excel** (no en chat, en su Excel):
   ...
   **Paso 4.3 — Aplicar la clasificación:**
   - Cuando el usuario diga "aplica la clasificación", ejecutar `python scripts/procesar_fotos.py --encargo <path> --aplicar-excel`.
   ...
   **Cómo se usa luego**: `redactar_v1.py` lee `fotos_descripciones.json` y usa la `Descripción` que escribió el usuario como **pie de imagen** ("Imagen NN: descripción") en el docx, en vez de derivar del nombre del archivo. Las fotos con `Deficiencia=general` no entran en ninguna sección 7.x — se reservan para que la skill las inserte conversacionalmente en Antecedentes o Portada si conviene.

   **Modo legacy** (sin Excel, dictando rangos): `--clasificar --map "1-12:DEF01:slug"`. Sigue funcionando pero no es el flujo recomendado.

   **Regenerar el Excel**: si el usuario añade más fotos a Drive después o quiere reiniciar la clasificación, ejecutar `--regenerar-excel` para reconstruir el .xlsx (atención: pierde lo que hubiera rellenado).
...
Tras la ingesta autónoma, mostrar al usuario un **resumen estructurado** con tres listas:
...
Pedir las respuestas en bloque (el usuario puede contestar todas seguidas en un solo mensaje o decir "no sé" donde no las tenga). Aplicar al `caso.yaml`.
...
Momento EXPLÍCITO del flujo donde se cierra la lista de deficiencias antes de redactar. Tres modos según cómo lo invoque el usuario:

**Modo A — Dictado:** el usuario dice algo tipo "son cuatro: humedades en el garaje, fisuras en fachada norte, problemas en la escollera del lindero sur, y filtraciones en la sala de calderas". Claude:
...
**Modo B — Editar el fichero:** el usuario ha editado `deficiencias.md` a mano (añadido, quitado o renombrado). Claude lo lee y sincroniza `caso.yaml > deficiencias`.

**Modo C — Aceptar la propuesta de `/informe ingestar`:** si el usuario dice "vale la propuesta" o similar, Claude lee el `deficiencias.md` que generó `ingestar` y lo sincroniza con `caso.yaml`.

En cualquier modo, al terminar:
- `deficiencias.md` queda como fuente de verdad legible paral usuario.
...
3. **escollera_sur** · Desprendimiento de ripios en escollera sur
   - Origen: dictada por el usuario, sin fotos asociadas todavía
...
- "Capítulo presupuesto" lo necesita la fase 2 — si el usuario no lo ha asignado, Claude lo deriva del slug en mayúsculas y deja el número correlativo.

**`/informe deficiencias` se puede ejecutar tantas veces como haga falta.** Si tras redactar el v1 el usuario descubre que falta una deficiencia, puede volver aquí, añadirla, y volver a `/informe redactar`.
...
Si falta algo, NO redactar — pedir al usuario que complete primero.
...
**Iteración con el usuario**: redactar deficiencia a deficiencia (no las 5 de golpe), enseñarle cada bloque, recoger correcciones, fijar y pasar a la siguiente. Esto da control y evita re-trabajo masivo.
...
**Tras ejecutar el script**: avisa al usuario que abra el docx y pulse F9 sobre el índice para regenerar la TOC, y mostrar el resumen de avisos del script.

### `/informe presupuestar` (o "inyecta el presupuesto")

**Pre-requisito:** el usuario ha exportado de Presto/Arquímedes **un fichero `.RTF`** a `99_salida/presupuesto/`. El nombre puede ser cualquiera — si hay varios, se usa el más reciente. Si hay versiones (p.ej. `..._02_02_...`, `..._03_03_...`), el script toma el último mtime.
...
1. `caso.yaml > deficiencias[i].subcapitulo_presupuesto: "04.05"` (código directo). Es el que la skill rellena al fijar deficiencias o como parte de la conversación con el usuario antes de presupuestar.
...
1. Pedir al usuario que confirme que ha exportado el RTF a `99_salida/presupuesto/`.
2. Parsear el RTF con `presupuestar.py` (en modo dry-run conceptual: leerlo y mostrar la lista de subcapítulos detectados).
3. Para cada deficiencia que NO tenga `subcapitulo_presupuesto` en `caso.yaml`, presentar al usuario los subcapítulos del RTF y pedirle que asigne el código (`04.05`, etc.). Aplicar al `caso.yaml` y a `deficiencias.md`.
...
**Tras ejecutar**:
- Mostrar al usuario el resumen del script (subcapítulos inyectados, costes en cuadro, hoja resumen).
...
Procesa una revisión hecha por el usuario en Word. Tres modos no exclusivos:
...
0. **Carga memoria externa primero** (`notas_por_documento.md`, `cronologia.md`, `briefing.md`, `caso.yaml`, `deficiencias.md`). Si una corrección del usuario exige verificar un dato del expediente, consultar esos ficheros antes que los PDFs.

1. **Lee `comentarios_revision.md`** y lo presenta al usuario en pantalla, comentario por comentario.
...
6. Si tras todo el usuario quiere otra versión, vuelve a `/informe revisar` partiendo del v(N+1).
...
├── 00_encargo/
│   ├── hoja_encargo.pdf            (subido por el usuario)
...
6. **`/informe nuevo` es interactivo y produce yaml COMPLETO.** No dejar campos "RELLENAR" salvo que el usuario expresamente diga "lo añado luego".
